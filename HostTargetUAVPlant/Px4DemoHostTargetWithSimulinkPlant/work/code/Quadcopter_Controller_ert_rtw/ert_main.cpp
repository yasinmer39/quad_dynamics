...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Quadcopter_Controller_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    extmodeSimulationTime_T currentTime = (extmodeSimulationTime_T)
      Quadcopter_Controller_M->Timing.taskTime0;
    Quadcopter_Controller_step();

    // Get model outputs here

    // Trigger External Mode event
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(Quadcopter_Controller_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Quadcopter_Controller_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  Quadcopter_Controller_terminate();
  extmodeReset();
...
{
  rtmSetErrorStatus(Quadcopter_Controller_M, 0);
  ...
  // Initialize model
  Quadcopter_Controller_initialize();

  // External Mode initialization
  errorCode = extmodeInit(Quadcopter_Controller_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Quadcopter_Controller_M, true);
    }
  }
